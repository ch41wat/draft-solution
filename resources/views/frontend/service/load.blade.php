<div class="row" id="load" style="position: relative;">
    ...
    <div class="col-md-8" style="background-image: url({{ asset('storage/uploads/' . $item->path . '/picture/' . $item->picture_name) }}); background-size: 100% 100%; background-repeat: no-repeat; min-height: 400px; width: 100%; margin-bottom: 10px;">
        @php $layer = 1 @endphp
        @for ($i = 1; $i <= 5; $i++)
            <div class="row">
            @for ($j = 1; $j <= 12; $j++)
                <div class="col-md-1 col-xs-1" style="min-height: 80px;">
